...
public class SqlSessionFactoryBean implements FactoryBean<SqlSessionFactory>,
	...
	private static final String KEY_GRANITE_MYBATIS_TYPE_ALIASES = "Granite-MyBatis-Type-Aliases";
	...
	private void scanBundle(Bundle bundle) {
		...
		String sTypeAliases = header.get(KEY_GRANITE_MYBATIS_TYPE_ALIASES);
		if (sTypeAliases != null) {
			NamedTypeAlias[] namedTypeAliases = scanTypeAliases(bundle, sTypeAliases);
	...
	private NamedTypeAlias[] scanTypeAliases(Bundle bundle, String sTypeAliases) {
		StringTokenizer st = new StringTokenizer(sTypeAliases, ",");
		NamedTypeAlias[] typeAliases = new NamedTypeAlias[st.countTokens()];
		
		int i = 0;
		while (st.hasMoreTokens()) {
			String sTypeAlias = st.nextToken();
			int equalMarkIndex = sTypeAlias.indexOf('=');
			
			String sType;
			String name = null;
			if (equalMarkIndex != -1) {
				name = sTypeAlias.substring(0, equalMarkIndex);
				sType = sTypeAlias.substring(equalMarkIndex + 1);
			} else {
				sType = sTypeAlias;
			}
