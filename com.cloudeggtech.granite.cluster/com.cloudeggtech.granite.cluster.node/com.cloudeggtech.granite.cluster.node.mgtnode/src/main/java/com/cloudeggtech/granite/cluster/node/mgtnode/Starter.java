...
import com.cloudeggtech.granite.cluster.node.mgtnode.deploying.pack.AppnodeRuntimesPacker;
import com.cloudeggtech.granite.cluster.node.mgtnode.deploying.pack.IAppnodeRuntimesPacker;
...
public class Starter implements Serializable {
	...
	private void packAppnodeRuntimes(Options options, DeployPlan plan) {
		...
		for (String nodeType : plan.getNodeTypes().keySet()) {
			String checksum = plan.getAppnodeRuntimeChecksum(nodeType);
