...
public class Main {
	...
	private Options parseOptions(String[] args) {
		...
		while (i < args.length) {
			if ("-update".equals(args[i])) {
				...
				if (i == (args.length - 1)) {
					throw new IllegalArgumentException("-repositorDir should follow a [REPOSITORY_DIR] option.");
				}
				i++;
				
				if (args[i].startsWith("-")) {
					throw new IllegalArgumentException("-repositoryDir should follow a [REPOSITORY_DIR] option.");
				}
	...
	private void printUsage() {
		...
		System.out.println("java -jar com.cloudeggtech.granite.deploy.cluster.mgtnode.pack-${VERSION}.jar [OPTIONS] [Bundle-SymbolicNames or SubSystems]");
